...
const encryptCookie = (data) => {
	try {
		...
		const encryptedBytes = (new aesjs.ModeOfOperation.ctr(new Uint8Array(key), new aesjs.Counter(5))).encrypt(dataBytes);
		return aesjs.utils.hex.fromBytes(encryptedBytes);
	} catch(e) {
		alert("Wystąpił błąd przy szyfrowaniu Twoich plików Cookie. Twoja sesja nie będzie w pełni bezpieczna!");
		return data;
	}
};
...
const decryptCookie = (enc) => {
	if (enc) {
		try {
			...
			// eslint-disable-next-line new-cap
			const decryptedBytes = (new aesjs.ModeOfOperation.ctr(new Uint8Array(key), new aesjs.Counter(5))).decrypt(encryptedBytes);
